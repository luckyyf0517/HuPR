...
def plotHumanPose(batch_joints, cfg, visDir, imageIdx, bbox=None, upsamplingSize=(256, 256), nrow=8, padding=2, truth=False):
    for j in range(len(batch_joints)):
        ...
        rgbPath = os.path.join(cfg.TEST.plotImgDir, 'single_%d'%int(namestr[:4]), 'kinect/imgs', '%04d.jpg'%int(namestr[-4:]))
        ...
        # ndarr = cv2.putText(ndarr, cfg.TEST.outputName, (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
